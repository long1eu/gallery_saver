...
class GallerySaver {
  ...
  static const String methodSaveAudio = 'saveAudio';
  ...
  static const String fileIsNotAudio = 'File on path is not an audio file.';
  ...
  ///saves audio from provided temp path and optional album name in gallery
  static Future<bool> saveAudio(String path) async {
    File tempFile;
    if (path == null || path.isEmpty) {
      throw ArgumentError(pleaseProvidePath);
    }
    if (!isAudio(path)) {
      throw ArgumentError(fileIsNotAudio);
    }
    if (!isLocalFilePath(path)) {
      tempFile = await _downloadFile(path);
      path = tempFile.path;
